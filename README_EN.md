...
👉 **[Live Demo](https://greeks-joinlearn.streamlit.app)** - Browse with sample data
...
- **Local deployment**: Only modify `.env` file (as explained above)
- **Streamlit Cloud deployment**: Refer to `secrets.toml.example` content and manually fill in Streamlit Cloud Secrets management interface

**Simple explanation**:
- `.env` = Configuration file for local use
- Streamlit Cloud Secrets = Configuration for cloud use
- `secrets.toml.example` = Reference documentation for cloud configuration
...
5. **Configure Secrets** (Settings → Secrets):
   ```toml
   # Refer to .streamlit/secrets.toml.example file
   # Only need to fill this one line to enable Demo mode
   ENABLE_DATA_COLLECTION = "false"
   ```
6. Deploy!

**Configuration notes**:
- **Don't** fill in Deribit API credentials (keep read-only Demo)
- `.streamlit/secrets.toml.example` is a reference template, no need to modify it
- Manually fill in Secrets on Streamlit Cloud web interface (as shown above)
